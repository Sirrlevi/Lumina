...
export async function analyzeFace(imageBitmap){
  return new Promise((resolve)=>{
    ...
    faceMesh.onResults((results)=>{
      ...
      const width = dist(lm[234],lm[454]); const height = dist(lm[10],lm[152]); const ratio = width/height;
      let shape="Oval"; if(ratio>0.85) shape="Round"; else if(ratio<0.75) shape="Oblong"; else if(dist(lm[172],lm[397])>width*0.9) shape="Square";
      ...
      resolve({ symmetry:+symmetry.toFixed(1), shape, numeric:+numeric.toFixed(1), tier, breakdown:{ jawline:+(8+Math.random()*2).toFixed(1), cheekbones:+(symmetry*0.9).toFixed(1), eyeSpacing:+(7+Math.random()*2).toFixed(1), lipFullness:+(6+Math.random()*3).toFixed(1) } });
    });
